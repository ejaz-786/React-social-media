...
const FeedsCard = ({ val }) => {
  ...
  return (
    ...
    >
      <CardHeader
        avatar={<Avatar>{val.by.charAt(0)}</Avatar>}
        action={
          <IconButton aria-label="settings">
            {logger.name.toLowerCase() !== val.by.toLowerCase() && (
              <Tippy
                content={
                  <i
      ...
      <CardActions disableSpacing>
        <IconButton aria-label="Likes" onClick={like}>
          {liked ? (
            ...
            <i className="fa-solid fa-heart" style={{ color: "grey" }} />
          )}{" "}
          {val.likes}
        </IconButton>
            ...
            {val.comments.map((val1, index) => {
              return (
                <ListItem key={index}>
                  <ListItemText
                    sx={{ width: "70%" }}
                    primary={`${Object.keys(val1)[0]} : ${
                      Object.values(val1)[0]
                    } `}
                  />
                  {Object.keys(val1)[0].toLowerCase() ===
                    logger.name.toLowerCase() && (
                    <>
                      <ListItemButton>
                        <i
